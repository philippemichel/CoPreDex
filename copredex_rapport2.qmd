...
author: 
    ...
    - name:  "Dr Édouard DEVAUD"
      affiliations:
        name: "Hôpital NOVO"
        department: "Médecine interne -- Site Pontoise"
lang: fr-FR
date: today
...
format: 
  titlepage-pdf:
    ...
    titlepage-footer: "Comparaison de la Prednisolone et de la Dexaméthasone sur la mortalité à J28 chez des patients sous oxygénothérapie, atteints de la CoViD-19\\newline Étude multicentrique, randomisée, de non-infériorité en ouvert"
titlepage-theme:
...
#
#| label: ittpp
pp <- read_csv("data/data2/exclus_itt.csv") |> 
  mutate(ID = str_sub(ID,1,6),show_col_types = FALSE)
ttitt <- ttz |> 
  dplyr::filter(!(patid %in% pp$ID), show_col_types = FALSE)
tt <- ttz
```
...
tt |> 
  select(c(3,4,7,9,10,12:19,21:33) ) |> 
...
```{r}
#| label: fact1
#| tbl-cap: Facteurs influants la survie
...
Trois atteintes rénales & une atteinte cutanée ont été notées.
...
La survie à 28 jours a été évaluée par un test du $\chi^2$ de Dunnet & Gent [@dunnet] en unilatéral plus adapté à une analyse de non infériorité.

Les survie ont été calculées par la méthode de Kaplan-Meyer puis  un test non-paramétrique du Log-rank en unilatéral a été tenté.
